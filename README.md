...
6. 手机多点巡逻：可用实时小车位姿或地图触点设置起点 `S`，再按顺序添加编号目标点，并分别调整起点和目标点方向。服务端先发布用户确认的起点 `/initialpose`，随后始终用连续的 `/navigate_to_pose` 逐点执行。每个点返回成功后还会读取 `map → base_footprint` 的实时 yaw；方向连续三次处于预设值 ±10° 内才进入下一点，否则重交当前目标进行一次方向校正，仍失败则停车并终止路线。DWA、TEB 和 A* + RPP 均使用同一流程。支持撤销、清空、追加返回起点、双指缩放、单指拖动和视图复位，障碍物或未知栅格上的触点会被拒绝。
